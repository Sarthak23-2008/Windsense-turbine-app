...
export const SimulationPage: React.FC<SimulationPageProps> = ({
  ...
  const [activeSubMode, setActiveSubMode] = useState<'uncontrolled' | 'feedback' | 'windsense'>('uncontrolled');

  // Re-evaluate simulation physics state whenever user adjusts parameters or wind speed
  const simState = evaluateWindSenseState(windSpeed, params);
    ...
    <div id="simulation-view" className="space-y-6">
      ...
      {activeSubMode === 'windsense' && (
        <div className="space-y-6">
          {/* Main Live Wind-Speed Simulator Bar */}
          <WindSpeedSimulator
            windSpeed={windSpeed}

...
import { Wind, Cog, Zap, Sparkles } from 'lucide-react';
...
export const LandingPage: React.FC<LandingPageProps> = ({ onLaunchDashboard }) => {
  ...
  return (
    <div id="landing-page-view" className="relative min-h-[calc(100vh-6rem)] flex flex-col justify-between overflow-hidden w-full px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
      
      {/* Launching Transition Overlay */}
      {isLaunching && (
        <div className="fixed inset-0 z-50 pointer-events-none flex items-center justify-center bg-slate-950/60 backdrop-blur-sm animate-pulse transition-opacity duration-500">
          <div className="absolute inset-0 bg-radial from-emerald-500/20 via-slate-950/40 to-slate-950/90" />
          <div className="relative flex flex-col items-center gap-4 text-center z-10 scale-100 transition-all duration-500">
            <div className="w-20 h-20 rounded-2xl bg-emerald-500/20 border-2 border-emerald-400/80 flex items-center justify-center shadow-[0_0_50px_rgba(16,185,129,0.7)] animate-bounce">
              <Cog className="w-10 h-10 text-emerald-400 animate-spin" />
            </div>
            <div className="space-y-1">
              <p className="text-emerald-400 font-mono text-sm tracking-widest uppercase font-bold">
                Initializing Telemetry
              </p>
              <p className="text-xs text-slate-400 font-mono">
                Connecting Digital Twin Simulation...
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 flex flex-col items-center text-center space-y-6 max-w-3xl mx-auto">
          ...
          <div className="pt-6 relative inline-flex items-center justify-center">
            ...
            <button
              ...
              className={`relative z-10 px-8 py-4 sm:px-10 sm:py-5 rounded-full bg-gradient-to-r from-emerald-500 via-teal-400 to-emerald-400 hover:from-emerald-400 hover:to-teal-300 text-slate-950 font-black text-base sm:text-xl shadow-[0_0_35px_rgba(16,185,129,0.5)] hover:shadow-[0_0_55px_rgba(16,185,129,0.8)] hover:scale-105 active:scale-95 transition-all duration-200 flex items-center justify-center gap-3 cursor-pointer group border border-emerald-200/60 ${isLaunching ? 'scale-105 opacity-90' : ''}`}
            >
              <Cog className={`w-6 h-6 text-slate-950 transition-transform shrink-0 ${isLaunching ? 'animate-spin' : 'group-hover:rotate-180 duration-700'}`} />
              <span className="tracking-wide">
                {isLaunching ? 'Launching Dashboard...' : 'Launch Live Dashboard →'}
              </span>
            </button>
          </div>

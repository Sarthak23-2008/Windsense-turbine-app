...
export type GovernorStateCategory = 'At Rest (0 RPM)' | 'Active (Preload Equilibrium)' | 'Active (Engaging)' | 'Active (Regulating)' | 'Active (Max Travel Limit)';
...
/**
 * Derives the visual Governor State category from continuous position percentage and live RPM.
 */
export function determineGovernorStateCategory(positionPct: number, rotorRpm: number = 0): GovernorStateCategory {
  if (rotorRpm <= 0) return 'At Rest (0 RPM)';
  if (positionPct <= 2.0) return 'Active (Preload Equilibrium)';
  if (positionPct < 25.0) return 'Active (Engaging)';
  if (positionPct <= 90.0) return 'Active (Regulating)';
  return 'Active (Max Travel Limit)';
}
...
export function calculateGovernorStep(
  ...
  const governorState = determineGovernorStateCategory(governorPositionPct, safeRpm);
